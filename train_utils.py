...
class EthnoHateTrainer():
    def __init__(self, model, train_loader, val_loader, configs, 
        ...
        if configs.opt in ['AdamW', 1]:
            self.opt = AdamW(params_to_train, lr=configs.lr)
        elif configs.opt in ['Adam', 2]:
    ...
    #отпралвяем модель на cpu или gpu
        ...
        if self.n_epochs==0:
            ...
            self.metrics_val[0] = plot_metrics(self.val_loader.dataset.nli_data.iloc[:,2].values.astype(int), 
                            self.answers_val[0].numpy().astype(int), 
                            self.metrics_val[0], 
                ...
                if j==0:
                    ...
                    if self.nli:
                        ...
                        for n,p in enumerate(self.configs.labels):
                            ...
                            accum += len(p)
                        #target = self.val_loader.dataset.nli_data.iloc[:,2].values
                else:
                    self.additional_train_loaders[j-1].dataset.epoch+=1
                    if not self.nli:
                        target = self.additional_val_loaders[j-1].dataset.data.iloc[:,1].values
                    else:
                        #self.answers_val[j] = nli_data.answers.values.reshape(len(data),len(nli_data)//len(data)).argmax(1)//len(configs.nli_labes[0])
                        target = self.additional_val_loaders[j-1].dataset.nli_data.iloc[:,2].values
...
def find_or_create_cache(configs):
    try:
        coding=None
        ...
        train_data, test_data = split_train_test(data, configs)
        val_data = test_data.sample(frac=configs.val_size, replace=False, random_state=228)
        test_data = test_data[~test_data.index.isin(val_data.index)]
...
def create_and_fit(model_name, configs, save = False, 
                 save_as_miner=False, best_metric=None, augs_only=False):
    ...
    train_data, test_data, _, coding = find_or_create_cache(configs)
    if configs.augment:
    ...
    if configs.multiple_tasks:
        ...
        for path in configs.tasks_data:
            ...
            val_loaders.append(DataLoader(EthnoHateDataset(test, configs, type='test'), 
                   batch_size=configs.batch_size, shuffle=False))
...
def error_analysis(model_path, val_data_path, batch_size=24, device='cuda'):
    ...
    if configs.nli:
        ...
        for n,p in enumerate(configs.labels):
            cond = torch.isin(torch.tensor(answers_val), torch.tensor([*range(accum, accum+len(p))]))
            answers_val[cond] = n
            accum += len(p)
    else:
        trues = val_loader.dataset.data.iloc[:,1].to_numpy()
