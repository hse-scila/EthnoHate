...
def split_train_test(data, config):
    ...
    if not config.ethno_specific:
        ...
        if not config.split_on_document:
            ...
            test_index = data.index.to_series().sample(frac=config.test_size, random_state=123)
            test = data.loc[test_index]
            train = data.drop(test_index, axis=0)
        
    return train, test

def get_resampler(target):
...
class EthnoHateDataset(Dataset):
    def __init__(self, data, configs=None, type='train'):
        self.data = data
        ...
        if configs is not None:
            ...
            if self.nli:
                print('Creating data for nli...')
                self.labels = configs.labels
                self.nli_template = configs.nli_template
                self.nli_data = pd.DataFrame(columns = ['premise', 'hypo', 'class', 'label'])
                for i in range(len(self.data)):
                    for j,l in enumerate(self.labels):
                        if l=='None':
                            ...
                            l = [l]
                        for k in l:
                            temp = pd.DataFrame({'premise':self.data.iloc[i,0],
                                     'hypo': self.nli_template.replace('[LABEL]', k),
                                     'class': int(j==self.data.iloc[i,1]),
                                     'label': self.data.iloc[i,1]}, 
                                      index = [self.data.index[i]])
                            self.nli_data = pd.concat([self.nli_data, temp])
    ...
    def __getitem__(self, idx):
        if self.nli:
            text, hypo, target, _, *rest = self.nli_data.iloc[idx]
